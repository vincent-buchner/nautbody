...
def test_play_speaker_opens_stream_with_configured_params(mock_pyaudio, pcm_bytes: bytes):
...
def test_play_speaker_writes_audio_bytes_to_stream(mock_pyaudio, pcm_bytes: bytes):
...
def test_play_speaker_reuses_existing_stream(mock_pyaudio, pcm_bytes: bytes):
...
def test_kill_speaker_stops_and_closes_stream(mock_pyaudio, pcm_bytes: bytes):
    _, mock_stream = mock_pyaudio
    audio_output = PyAudioOutput(sample_rate=16_000, channels=1)
    audio_output.play_speaker(pcm_bytes)
...
def test_kill_speaker_without_stream_is_a_noop(mock_pyaudio):
    audio_output = PyAudioOutput(sample_rate=16_000, channels=1)
