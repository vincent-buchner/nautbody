...
def groq_api_key(monkeypatch: pytest.MonkeyPatch):
...
def mock_groq():
    with patch("src.infrastructure.llm_provider.groq.Groq") as mock_cls:
        ...
        mock_client.chat.completions.create.return_value = _mock_completion("mocked response")
        yield mock_cls, mock_client


def test_init_creates_client_with_api_key_from_env(mock_groq):
...
def test_generate_response_returns_message_content(mock_groq):
    _, mock_client = mock_groq
    mock_client.chat.completions.create.return_value = _mock_completion("hello there")
    provider = GroqProvider(system_prompt="you are a helpful assistant", model_config=MODEL_CONFIG)
...
def test_generate_response_returns_none_when_no_content(mock_groq):
    _, mock_client = mock_groq
...
def test_generate_response_sends_system_and_user_messages(mock_groq):
    _, mock_client = mock_groq
    provider = GroqProvider(system_prompt="you are a helpful assistant", model_config=MODEL_CONFIG)
...
def test_generate_response_accumulates_conversation_history(mock_groq):
    _, mock_client = mock_groq
